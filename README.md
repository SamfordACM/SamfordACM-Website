...
Requires the exact Node version in `.nvmrc` (currently 24.11.1).

The version is pinned deliberately. `npm ci` requires the lockfile to have been
generated by a compatible npm, and npm 12 changed what it expects in the tree --
an unpinned `24` let CI pick up a newer npm than anyone had locally, and the
build failed on missing lockfile entries. To upgrade Node later, bump `.nvmrc`,
run `npm install` with that version to regenerate `package-lock.json`, and
